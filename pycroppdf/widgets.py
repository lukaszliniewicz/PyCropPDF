    ...
    def mouseMoveEvent(self, event):
        ...
        if self._mode == "none":
            ...
            if "top" in self._resize_handle:
                new_rect.setTop(self._original_rect.top() + delta.y())
            if "bottom" in self._resize_handle:
                new_rect.setBottom(self._original_rect.bottom() + delta.y())
            if "left" in self._resize_handle:
                new_rect.setLeft(self._original_rect.left() + delta.x())
            if "right" in self._resize_handle:
                new_rect.setRight(self._original_rect.right() + delta.x())
